require 'two_round_election'

RSpec.describe TwoRoundElection do

  let(:election) { TwoRoundElection.new }
  ...
  describe "#winner" do
    ...
    context "absolute majority" do

      it "is the candidate having more than half the total votes" do
        ...
        election.rank(count: 7_818, candidates: [cecile, basile, emilie, didier, albert])
    ...
    context "several candidates" do
      ...
      it "is the candidate elected by alternative vote" do
        ...
        election.rank(count: 364,   candidates: [emilie, cecile, didier, basile, albert])
        expect(election.winner).to eq(basile)
      end
